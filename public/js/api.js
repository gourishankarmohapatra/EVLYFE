// API Layer - EVLYFE (Fetches from server-side API routes)
const API = {
  vehicles: [],
  brands: [],
  dealers: [],
  ...
  async loadData() {
    try {
      const [vehiclesRes, brandsRes, dealersRes, blogsRes, comparisonsRes, faqsRes, upcomingRes, chargingRes, companiesRes] = await Promise.all([
        ...
        fetch('/api/companies?limit=300')
      ]);

      const vehiclesData = await vehiclesRes.json();
      this.vehicles = vehiclesData.docs || vehiclesData;
      this.brands = (await brandsRes.json());
      if (!Array.isArray(this.brands)) this.brands = this.brands.brands || [];
      ...
      this.companies = companiesData.docs || companiesData.companies || [];
